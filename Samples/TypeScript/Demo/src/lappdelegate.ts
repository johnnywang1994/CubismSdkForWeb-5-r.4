...
export class LAppDelegate {
  ...
  /**
   * Resize canvas according to size configuration.
   */
   ...
   * APPに必要な物を初期化する。
   */
  public initialize(options?: any): boolean {
    let canvas = typeof options.el === 'string'
      ? document.querySelector(options.el)
      : document.createElement('canvas');
    // Store canvas size and quality configuration
    this._canvasSize = options.size || 'auto';
    this._quality = options.quality || 1;
  ...
  public onResize(): void {
    this._resizeCanvas(this._canvases.at(0));
    this.initialize();
  ...
  private initializeSubdelegates(canvas?: HTMLCanvasElement): void {
    ...
    if (canvas) {
      ...
      // Create canvas elements (default behavior)
      ...
      for (let i = 0; i < LAppDefine.CanvasNum; i++) {
        ...
        this._canvases.pushBack(newCanvas);
        newCanvas.style.width = `${width}vw`;
        newCanvas.style.height = `${height}vh`;
    ...
    // Apply size configuration to all canvases
    for (let i = 0; i < this._canvases.getSize(); i++) {
      const targetCanvas = this._canvases.at(i);
      this._resizeCanvas(targetCanvas);
    }
   ...
   */
  private _canvasSize: string | { width: number; height: number };

  /**
   * Rendering quality multiplier
   */
