...
export class LAppModel extends CubismUserModel {
  /**
   * Encode URL path to handle special characters like # and spaces
   * This prevents # from being interpreted as a fragment identifier
   * Only encode the filename part, not the directory structure
   * @param urlPath The complete URL or path including domain and query parameters
   * @returns Properly encoded URL path
   */
  public static encodeUrlPath(urlPath: string): string {
    console.log('[encodeUrlPath] Input:', urlPath);
    
    try {
      // Split path into parts and only encode the filename (last part)
      const pathParts = urlPath.split('/');
      if (pathParts.length > 0) {
        const lastPart = pathParts[pathParts.length - 1];
        if (lastPart) {
          // Only encode the filename part using encodeURIComponent
      ...
      console.log('[encodeUrlPath] Output:', result);
      return result;
    } catch (e) {
      console.error('[encodeUrlPath] Error:', e);
      return urlPath;
    }
  }
  ...
  private setupTextures(): void {
    ...
    if (this._state == LoadStep.LoadTexture) {
      ...
      for (
        ...
        console.log('[LAppModel] Raw texture filename from JSON:', texturePath);
        ...
        console.log('[LAppModel] Encoded full texture path:', texturePath);
